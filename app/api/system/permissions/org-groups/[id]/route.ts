...
export async function GET(
    ...
    try {
        ...
        const group = await prisma.orgUserGroup.findUnique({
            where: { id },
            include: {
                permissions: true,
                _count: {
        ...
        console.error("[API] Error fetching org permission group:", error)
...
export async function PATCH(
    ...
    try {
        ...
        const existingGroup = await prisma.orgUserGroup.findUnique({
            where: { id }
        })
        ...
        if (name && name !== existingGroup.name) {
            const nameExists = await prisma.orgUserGroup.findUnique({
                where: { name }
            })
            if (nameExists) {
        ...
        const updatedGroup = await prisma.orgUserGroup.update({
            where: { id },
            data: {
                name,
        ...
        console.error("[API] Error updating org permission group:", error)
        return new NextResponse("Internal Server Error", { status: 500 })
    }
}
...
export async function DELETE(
    ...
    try {
        ...
        await prisma.orgUserGroup.delete({
            where: { id }
        })

        return new NextResponse(null, { status: 204 })
    } catch (error) {
        console.error("[API] Error deleting org permission group:", error)
        return new NextResponse("Internal Server Error", { status: 500 })
    }
}
